...
function add_node_to_dns($ip, $port, $version) {
	...
	if (!empty($ip) && ip2long($ip) != 0 && $version >= $CONFIG['MIN_VERSION'] && $port == 10332)
...
function connect_to_db() {
	...
	$db->exec("CREATE TABLE IF NOT EXISTS nodes (
			ipv4 INT NOT NULL,
			port INT NOT NULL DEFAULT 10332,
...
function get_list_of_nodes_for_dns() {
	global $db, $CONFIG;
	return $db->query("SELECT ipv4 FROM nodes WHERE accepts_incoming = 1 AND port = 10332 AND version >= ".$CONFIG['MIN_VERSION']." ORDER BY last_check DESC LIMIT 20;");
}

// Functions used only by count-nodes.php
function query_version_count() {
	global $db, $CONFIG;
	return $db->query("SELECT COUNT(*), version FROM nodes WHERE accepts_incoming = 1 AND port = 10332 GROUP BY version ORDER BY version;");
}

function query_dns_total() {
	global $db, $CONFIG;
	return $db->query("SELECT COUNT(*) FROM nodes WHERE accepts_incoming = 1 AND port = 10332 AND version >= ".$CONFIG['MIN_VERSION'].";");
}
